"""Tests for app/normalize.py -- pure logic, no DB dependency. Same
tests as Elektrica's test_normalize.py (identical module, copied
deliberately, see app/normalize.py's own docstring).
...
def test_normalize_phone_leading_country_code_not_stripped():
    ...
    # real data. Asserting the CURRENT documented behavior so a future
    # change to this is a deliberate, visible diff, not a silent regression.
    check(
